...
class UserController {
  ...
  static async loginHandler({ email, password }) {
    email = email.toLowerCase();
    const customer = await findOne({ email: email });
    if (_.isNil(customer)) {
      throw new ApiError('1003', errors.NOT_FOUND.status);
    } else if (customer.isDeleted) {
      throw new ApiError('1004', errors.NOT_ALLOWED.status);
    } else if (!(await this.comparePassword(password, customer.password)))
      throw new ApiError('1005', errors.NOT_ALLOWED.status);
    else {
